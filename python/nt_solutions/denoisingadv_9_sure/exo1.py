...
for i in range(nlaunch):
    ...
    f = np.tile(f0, (ntrials,1,1)) + sigma*random.standard_normal((ntrials,n,n))
    hf = h(f,lambd)
    
    #quadratic error
    e = np.sum((hf - np.tile(f0,(ntrials,1,1)))**2,(1,2))
    E0 = np.hstack((E0,e)) 
    
    #sure error
    e = -N*sigma**2 + np.sum((hf-f)**2,(1,2)) + 2*sigma**2*df(lambd)
    E = np.hstack((E,e)) 
