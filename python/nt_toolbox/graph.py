...
def perform_dijstra_fm(
    W, pstart, niter=np.inf, method="dijstr", bound="sym", svg_rate=10
):
    """
    ...
    n = W.shape[0]
    pstart = np.asarray(pstart, dtype=int)
        ...
        for j in J:
            ...
            if method == "dijstr":
                ...
                if Delta >= 0:
                    ...
                    D[u[0], u[1]] = min(dx + w, dy + w)
        if iter % svg_rate == 0 and saved < q:
            Dsvg[:, :, saved] = D
            Ssvg[:, :, saved] = S
            saved += 1

    return D, Dsvg[:, :, :saved], Ssvg[:, :, :saved]
