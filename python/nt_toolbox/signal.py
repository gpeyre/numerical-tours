...
def bilinear_interpolate(im, x, y):
    """Bilinear interpolation with coordinates clamped to image boundaries."""
    x = np.clip(np.asarray(x, dtype=float), 0, im.shape[1] - 1)
    y = np.clip(np.asarray(y, dtype=float), 0, im.shape[0] - 1)
    x0 = np.floor(x).astype(int)
    y0 = np.floor(y).astype(int)
    x1 = np.minimum(x0 + 1, im.shape[1] - 1)
    y1 = np.minimum(y0 + 1, im.shape[0] - 1)
    dx, dy = x - x0, y - y0
    return (
        (1 - dx) * (1 - dy) * im[y0, x0]
        + (1 - dx) * dy * im[y1, x0]
...
def plot_dictionary(D, title="Dictionary"):
    ...
    padding = ((pad_size, pad_size), (pad_size, pad_size), (0, missing_atoms))
